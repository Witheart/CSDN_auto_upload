...
- **🔁 保存自动重试**：点击"保存草稿"后会轮询校验结果，遇到"操作过于频繁"等频控提示会自动按指数退避等待并重试（最多 8 次），不再需要手动反复点击；确认保存成功后才记账。
...
- **频控自动处理**：保存遇到"操作过于频繁"时脚本会自动等待并重试（最长等待 60 秒），期间请勿最小化浏览器窗口；若 8 次重试仍未成功，脚本会提示手动检查且**不会记账**。
